...
jbot_botset=$root/data/config/bot.json
diybot_repo=$root/data/repo/diybot
diybot_diy=$root/data/jbot/diy
diybot_repo_diybotset=$root/data/repo/diybot/config/diybotset.json
diybot_config_diybotset=$root/data/data/config/diybotset.json
diybot_repo_user=$root/data/repo/diybot/jbot/diy/user.py
diybot_diy_user=$root/data/jbot/diy/user.py
diybot_repo_diy=$root/data/repo/diybot/jbot/diy/diy.py
diybot_diy_diy=$root/data/jbot/diy/diy.py
diybot_repo_hello=$root/data/repo/diybot/jbot/__main__.py
diybot_diy_hello=$root/data/jbot/__main__.py
if [ $# = 1 ];then
  up=$1
else
  up=no
...
copy() {
  ...
  cp -rf $root/data/repo/diybot/jbot/diy/* $root/data/jbot/diy
}
  ...
  if [ -z $(grep -E "123456789" $jbot_botset) ]
    then if [ -d "/jd" ]
      then cd $dir_jbot
        ...
        nohup python3 -m jbot >$root/data/log/bot/bot.log 2>&1 &
      fi
  else
